...
This vignette discusses the basics of the scalable estimation of consideration set models the ```ScalableCS``` package in Matlab. The background article for it is ["Scalable Estimation of Multinomial Response Models with Random Consideration Sets", Journal of the American Statistical Association, by Siddhartha Chib and Kenichi Shimizu](https://www.tandfonline.com/doi/full/10.1080/01621459.2025.2609361).
...
* $b_i\sim N(0, D)$ is a vector of random effects (by default, the code assigns random effects to all covariates),
...
* Practically, this amounts to clustering the units into latent groups. In each group, consideration or attention to different products are independent and the <em>attention probabilities</em> are shared among the units.
...
* The dataset should contain two matrices: the response matrix ```YData``` and the covariate matrix ```XData```.

### The response matrix   
* ```YData``` contains information on the responses $y_{ijt}$.
...
* ```XData``` contains information on covariates $x_{ijt}$.
...
To use the package ```ScalableCS```, simply download the folder ```ScalableCS``` and save it locally. 
...
## Price sensitivity
* By default, the code provides sensitivity measure of demand/response with respect to the first covariate in ```XData```, in this case, price.
